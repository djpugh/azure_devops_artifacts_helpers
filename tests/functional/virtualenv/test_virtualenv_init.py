...
class VirtualEnvTestCase(unittest.TestCase):
    ...
    def test_venv_pip(self):
        subprocess.check_call([sys.executable, '-m', 'virtualenv', str(Path(self.td)/TEST_VENV_NAME), '--seeder', 'azdo-pip'])
        installed_packages = subprocess.check_output([str(Path(self.td)/pip_path), 'freeze']).decode()
